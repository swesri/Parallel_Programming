...
/*---------------------------------------------------------------------------------------------------------------*/
  ...
  dim3 dimGrid(b);   
    ...
    cudaError_t error;
  /*  error = cudaMalloc((void**)&d_E, cmdLineArgs.P*b*sizeof(double));
    if (error != cudaSuccess) Cleanup(false);
    error = cudaMalloc((void**)&d_P1, cmdLineArgs.P*b*sizeof(double));
    if (error != cudaSuccess) Cleanup(false);
    error = cudaMalloc((void**)&d_Y, cmdLineArgs.P*b*sizeof(double));
    if (error != cudaSuccess) Cleanup(false);*/

    cudaMalloc( (void**)&d_E, sizeof(double*)*b );
    cudaMalloc( (void**)&d_P1, sizeof(double*)*b );
    cudaMalloc( (void**)&d_Y, sizeof(double*)*b );
   /* double* tmp_ptr1[b];
    double* tmp_ptr2[b];
    double* tmp_ptr3[b]; 
    for(int i=0; i<b; i++)
    	cudaMalloc( (void**)&tmp_ptr1[i], sizeof(double)*cmdLineArgs.P);
    cudaMemcpy(d_E, tmp_ptr1, sizeof(tmp_ptr1), cudaMemcpyHostToDevice);
    for(int i=0; i<b; i++)
    	cudaMalloc( (void**)&tmp_ptr2[i], sizeof(double)*cmdLineArgs.P);
    cudaMemcpy(d_P1, tmp_ptr2, sizeof(tmp_ptr2), cudaMemcpyHostToDevice);
    for(int i=0; i<b; i++)
    	cudaMalloc( (void**)&tmp_ptr3[i], sizeof(double)*cmdLineArgs.P);
    cudaMemcpy(d_Y, tmp_ptr3, sizeof(tmp_ptr3), cudaMemcpyHostToDevice);*/
...
/*---------------------------------------------------------------------------------------------------------------*/
  ...
  cudaMalloc( (void**)&E[i], sizeof(double)*cmdLineArgs.P);
  P[i]	  = (double*)malloc(cmdLineArgs.P*sizeof(double));
  //P1[i]    = (double*)malloc(cmdLineArgs.P*sizeof(double));
 cudaMalloc( (void**)&P1[i], sizeof(double)*cmdLineArgs.P);
  ...
  for (long t=0; t<cmdLineArgs.iter; t++) //Time loop
  {
	 for (long s=0; s<k2; s++) //Bunch loop
	  { 	
		...
		//for(int i=0; i<b; i++){
		error = cudaMemcpy(d_P1, P1,sizeof(double*)*b, cudaMemcpyHostToDevice);
		if (error != cudaSuccess) {printf("Error=%d\n",error);Cleanup(false);}
		error = cudaMemcpy(d_Y, Y,sizeof(Y), cudaMemcpyHostToDevice);
		if (error != cudaSuccess) Cleanup(false);//}
		error = cudaMemcpy(d_E, E,sizeof(double*)*b, cudaMemcpyHostToDevice);
		if (error != cudaSuccess) Cleanup(false);
		 printf("Hello110\n");
		  //displayMatrix ("device P1", d_P1,b,cmdLineArgs.P);
		   // displayMatrix ("host P1", P1,b,cmdLineArgs.P);
			 // displayMatrix ("device Y", Y,b,cmdLineArgs.P);
			   // displayMatrix ("host Y", Y,b,cmdLineArgs.P);
		AddVectors<<<dimGrid, dimBlock>>>(d_E, d_P1, d_Y);
cudaDeviceSynchronize();
printf("Hello2\n");
		double** E2    = (double**)malloc(b*sizeof(double*));
		cudaMemcpy(E, d_E,sizeof(double), cudaMemcpyDeviceToHost);
		for(int i=0; i<b; i++){
		//E2[i]    = (double*)malloc(cmdLineArgs.P*sizeof(double));
		double* host = E2[i];
		double* device = E[i];
        //error = cudaMemcpy(host, device,sizeof(double)*cmdLineArgs.P, cudaMemcpyDeviceToHost);
	error = cudaMemcpy(E, d_E,sizeof(double), cudaMemcpyDeviceToHost);
	printf("i=%d\n",i);
   		if (error != cudaSuccess) Cleanup(false);}
		displayMatrix ("error", E,b,cmdLineArgs.P);
